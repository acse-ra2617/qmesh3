...
class TestGlobe(unittest.TestCase):
    ...
    def create_equator_shapefiles(self):
        ...
        try:
            equatorLine = qmesh3.vector.loxodromicLine( \
                                startPoint = (-179.,0.), \
                                trueNorthBearing = 90., \
                                numbPoints=20, \
                                loopArounds = 0, \
    ...
    def create_circular_island(self):
        ...
        try:
            islandShoreline = qmesh3.vector.Circle(
                                centerPointXi = 0.0,
                                centerPointEta = -20.0,
                                radius = 10,
                                numbPoints = 20,
    ...
    def test_halfGlobe(self):
        ...
        #read-in Shapefiles
        try:
            boundary = qmesh3.vector.Shapes()
            boundary.fromFile(self.equatorLine_filename)
            loopShapes = qmesh3.vector.identifyLoops(boundary,
                      isGlobal=True, defaultPhysID=1, fixOpenLoops=True, extraPointsPerVertex=3)
            polygonShapes = qmesh3.vector.identifyPolygons(loopShapes,
